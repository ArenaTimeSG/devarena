-- Migração completa para criar a tabela settings
-- Esta migração resolve o problema do hook useSettings e initializeSettings

-- 1. Remover objetos existentes para evitar conflitos
...
-- 2. Criar a tabela settings com todas as colunas especificadas
...
BEGIN
    ...
    -- Relatar status
    RAISE NOTICE '=== VERIFICAÇÃO DA TABELA SETTINGS ===';
...
-- 13. Testar inserção de configurações (opcional - para verificação)
DO $$
DECLARE
    test_user_id UUID;
    test_settings_id UUID;
BEGIN
    -- Verificar se há pelo menos um usuário para teste
    ...
    IF test_user_id IS NOT NULL THEN
        ...
        INSERT INTO public.settings (
            ...
            test_user_id,
            '{"volei": true, "futsal": false, "basquete": true}'::jsonb,
            '{"volei": "#ff0000", "futsal": "#00ff00", "basquete": "#0000ff"}'::jsonb,
            '{"monday": {"start": "09:00", "end": "21:00", "enabled": true}}'::jsonb,
            30,
            '{"email": false, "push": true}'::jsonb,
            'dark',
            '{"name": "Teste", "email": "teste@teste.com"}'::jsonb
        ) ON CONFLICT (user_id) DO UPDATE SET
            modalities_enabled = EXCLUDED.modalities_enabled,
            modalities_colors = EXCLUDED.modalities_colors,
        ...
        RAISE NOTICE '✅ Teste de inserção/atualização bem-sucedido. ID: %', test_settings_id;
    ELSE
        RAISE NOTICE 'ℹ️ Nenhum usuário encontrado para teste de inserção';
    END IF;
END $$;
