...
serve(async (req) => {
  console.log('🚀 WEBHOOK SIMPLES - Method:', req.method);
  console.log('🚀 WEBHOOK SIMPLES - URL:', req.url);
  console.log('🚀 WEBHOOK SIMPLES - Headers:', Object.fromEntries(req.headers.entries()));
  ...
  try {
    ...
    console.log('✅ WEBHOOK SIMPLES PROCESSADO COM SUCESSO - Retornando 200 OK');
    return new Response("ok", { status: 200, headers: corsHeaders });

  } catch (error) {
    console.error('❌ Erro no webhook simples:', error);
    return new Response(
